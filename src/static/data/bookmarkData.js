export const bookmarkData = [
  {
    title: '搜索',
    list: [
      {name: 'Suemor搜索', url: 'https://search.suemor.com/'},
      {name: '思谷搜索', url: 'https://siguso.com/'},
      {name: 'Here200搜索', url: 'https://here200.github.io/Navigation/'},
      {name: '小兔爱搜', url: 'https://go.xl0408.top/'},
    ]
  },
  {
    title: '设计视觉',
    list: [
      {name: '阿里图标', url: 'https://www.iconfont.cn/'},
      {name: '极简壁纸', url: 'https://bz.zzzmh.cn/index'},
    ]
  },
  {
    title: '影音阅读',
    list: [
      {name: 'FreeMP3', url: 'https://tools.liumingye.cn/music/'},
      {name: 'Bilibili', url: 'https://www.bilibili.com/'},
      {name: 'YouTube', url: 'https://www.youtube.com/'},
      {name: 'ACFun', url: 'https://www.acfun.cn/'},
      {name: '电视眼', url: 'http://www.tvyan.com/'},
      {name: 'CCTV节目网', url: 'http://tv.cctv.com/live/'},
    ]
  },
  {
    title: '开发',
    list: [
      {name: 'CSDN', url: 'https://www.csdn.net/'},
      {name: 'Gitee', url: 'https://gitee.com/'},
      {name: 'Github', url: 'https://github.com/'},
      {name: '知乎', url: 'https://www.zhihu.com/'},
      {name: '掘金', url: 'https://juejin.cn/'},
      {name: '思否', url: 'https://segmentfault.com/'},
      {name: 'StackOverflow', url: 'https://stackoverflow.com/'},
      {name: 'Wiki', url: 'https://zh.wikipedia.org/'},
    ]
  },
  {
    title: '音视频解析',
    list: [
      {name: 'HHTJim’s部落格 Web App', url: 'https://link.hhtjim.com/'},
      {name: '够低调解析', url: 'http://www.gddyu.com/'},
      {name: '夜幕解析', url: 'https://yemu.xyz/'},
      {name: 'VIP视频在线解析', url: 'https://v.ctrlqq.com/'},
      {name: '热播之家', url: 'https://rebozj.com/'},
      {name: 'Here200视频解析', url: 'https://here200.github.io/Watch-Video-By-Interface/'},
    ]
  },
  {
    title: '资源',
    list: [
      {name: '影猫の仓库', url: 'https://ymck.me/'},
    ]
  },
  {
    title: '友链',
    list: [
      {name: 'Here200\'s Blog', url: 'https://here200.github.io/'},
      {name: 'M了个J', url: 'https://www.cnblogs.com/mjios'},
      {name: 'Suemor\'s Blog', url: 'https://suemor.com/'},
      {name: '超级小兔', url: 'https://xl0408.top/'},
    ]
  },
]